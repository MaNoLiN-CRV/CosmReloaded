...
export default function People({ people }: PeopleProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {people.map((person) => {
        const sharedClassName = "glass-item group hover:scale-105 transition-transform flex items-center gap-x-4 p-6";
        const content = (
          <>
            {person.image && (
        ...
        return person.github ? (
          ...
          <div key={person.name} className={sharedClassName}>
            {content}
          </div>
        );
      })}
